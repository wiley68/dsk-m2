...
function dskapi_buildGetProductCustomUrl(
  dskapi_cid,
  dskapi_price,
  dskapi_product_id,
  dskapi_vnoski,
) {
  const baseUrlEl = document.getElementById("dskapi_getproductcustom_url");
  const baseUrl = baseUrlEl ? baseUrlEl.value : "";
  const params =
    "cid=" +
    encodeURIComponent(dskapi_cid) +
    "&price=" +
    encodeURIComponent(dskapi_price) +
    "&product_id=" +
    encodeURIComponent(dskapi_product_id) +
    "&dskapi_vnoski=" +
    encodeURIComponent(dskapi_vnoski);
  ...
  return DSKAPI_LIVEURL + "/function/getproductcustom.php?" + params;
}
...
function dskapi_pogasitelni_vnoski_input_change() {
  ...
  var xmlhttpro = createCORSRequest(
    "GET",
    dskapi_buildGetProductCustomUrl(
      dskapi_cid,
      dskapi_price,
      dskapi_product_id,
      dskapi_vnoski,
...
function calculateDskapiPriceWithOptions(
  ...
  if (typeof offers[0] !== "undefined") {
    const span1 = offers[0].querySelectorAll('[data-price-type="finalPrice"]');
...
function updateDskapiButtonValues() {
  ...
  if (dskapi_vnoski_txt) {
    ...
    if (dskapi_vnoski_txt.textContent) {
      ...
      defaultVnoski = parseInt(dskapi_pogasitelni_vnoski_input.value) || 12;
    }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (btn_dskapi !== null) {
    ...
    const dskapiSpanOffers = document.querySelectorAll('[itemprop="offers"]');
    if (typeof dskapiSpanOffers[0] !== "undefined") {
      ...
      if (typeof dskapiSpanFinalPrice[0] !== "undefined") {
        ...
        if (dskapiTargetNode !== null && dskapiTargetNode instanceof Node) {
